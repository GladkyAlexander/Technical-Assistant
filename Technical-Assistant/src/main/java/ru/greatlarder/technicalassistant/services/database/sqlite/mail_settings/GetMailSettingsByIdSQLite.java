...
public class GetMailSettingsByIdSQLite implements GetMailSettings {
    @Override
    public MailSettings getMailSettings(User user, String idMailSettings) {
        createMailSettingsTable();
        try {
            resultSet = statement.executeQuery(SQLiteMailSettings.READ_TABLE_MAIL_SETTINGS);
            while (resultSet.next()){
                if(Objects.equals(resultSet.getInt("id"), Integer.valueOf(idMailSettings))){
