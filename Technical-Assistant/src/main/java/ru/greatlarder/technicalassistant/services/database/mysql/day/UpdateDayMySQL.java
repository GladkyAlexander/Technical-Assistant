...
import ru.greatlarder.technicalassistant.services.database.UpdateDay;

import java.sql.Date;
import java.sql.PreparedStatement;
import java.sql.SQLException;

public class UpdateDayMySQL implements UpdateDay {
    @Override
    public void updateDay(User user, Day day) {
        ...
        try {
            PreparedStatement cf = connectMySQL.connectionMySQL.prepareStatement(daysTableMySQL.UPDATE(user.getNameDB()));
            cf.setInt(1, day.getId());
            cf.setString(2, day.getRoom());
            cf.setDate(3, Date.valueOf(day.getDate()));
            if (day.getA800() != null) {
