...
public class BookingEquipmentByLocalDateAndEquipmentMySQL implements GetBookingEquipment {

    @Override
    public BookingEquipment getBookingEquipment(User user, LocalDate lastDate, String idEquipment) {
        ...
        try {
            ...
            while (connectMySQL.resultSetMySQL.next()){
                if(Objects.equals(connectMySQL.resultSetMySQL.getInt("equipmentId"), Integer.valueOf(idEquipment))
                        && connectMySQL.resultSetMySQL.getDate("date").toLocalDate().equals(lastDate)){
