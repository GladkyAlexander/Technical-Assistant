...
import ru.greatlarder.technicalassistant.services.database.sqlite.equipment.ListEquipmentByNameRoomSQLite;
import ru.greatlarder.technicalassistant.services.list_view.GetListViewEquipment;

import java.io.IOException;

public class ListViewEquipmentByNameRoomSQLite implements GetListViewEquipment {
    @Override
    public ListView<Equipment> getListViewEquipment(User user, String nameCompany, String nameRoom) {

        GetListEquipment getListEquipment = new ListEquipmentByNameRoomSQLite();
        ListView<Equipment> listView = new ListView<>(FXCollections.observableArrayList(getListEquipment
                .getListEquipment(user, nameCompany, nameRoom)));
