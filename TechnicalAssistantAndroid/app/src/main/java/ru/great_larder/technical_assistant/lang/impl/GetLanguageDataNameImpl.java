...
import ru.great_larder.technical_assistant.lang.GetLanguageDataName;
...
public class GetLanguageDataNameImpl implements GetLanguageDataName {
    ...
    public String getDayWeek(LocalDate date, String lang) {
        ...
        if(lang.equals(ru)) if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            return hashMapRu.get(String.valueOf(date.getDayOfWeek()));
        }
        if(lang.equals(en)) if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            return hashMapEn.get(String.valueOf(date.getDayOfWeek()));
        }
